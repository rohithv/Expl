...
%token NUM TXT PLUS MINUS MUL DIV ASSIGN WRITE READ ID BEGINTK END EQ NE LT GT LE GE IF THEN ELSE ENDIF WHILE DO ENDWHILE BREAKTK CONTINUETK BRKPTK REPEAT UNTIL DECL ENDDECL INT STR ADDRTK MAINTK RETURN ANDTK ORTK NOTTK TUPLE TYPETK ENDTYPE NULLTK ALLOC INITIALIZE FREE MOD
%left PLUS MINUS
%left MUL DIV MOD
...
LDeclBlock : DECL LDeclList ENDDECL { /* No Action */createLsymbolEntries(paramlist);   }
			| DECL ENDDECL { /* No Action */createLsymbolEntries(paramlist);   }
			| { createLsymbolEntries(paramlist);   }
			;
			
LDeclList : LDeclList LDecl {  /* No Action */ }
	   | LDecl {  /* No Action */ }
	   ;
...
expr :	expr PLUS expr 	  {$$=createTree(0,TLookup("int"),NULL,PLUSOP,$1,NULL,$3);}
	...
	| ID '(' ')' { $$=createTree($1->val, NULL,$1->varname, FUNCTION, NULL, NULL, NULL); free($1); }
...
Iden :ID '[' expr ']'  { $$=createTree($3->val,$1->type,$1->varname,$1->nodetype,$3,$1->middle,$1->right); //typecheck($$);
				//free($3);
				free($1); }
