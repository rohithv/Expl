...
TypeDefList : TypeDefList TypeDef { /*Empty*/ }
			 | TypeDef { /*Empty*/ }
			 ;
...
FDefBlock : FDefBlock FDef { /*Empty*/ }
		  | FDef{ /*Empty*/ }
		  ;
...
FieldFunction : SELF '.' ID '(' ArgList ')'	{ if(currclass==NULL){ fprintf(stderr,"error %d : self can only be used inside a class definition\n",line); exit(1);}
											  MemberFuncList *mfunc=checkMemberFunc(currclass, $3->varname, $5);
			  ...
			  | ID '.' ID '(' ArgList ')' 	{ typecheck($1);
											  MemberFuncList *mfunc=checkMemberFunc($1->ctype, $3->varname, $5);
			  ...
			  | Field '.' ID '(' ArgList ')'{ tnode *tmp;
											  ...
											  mfunc=checkMemberFunc(tmp->ctype, $3->varname, $5);
